...
- [x] **BACKUP-04**: React `useAnonAuth` hook surfaces `passkeyBackedUp: boolean | null` and `passkeyBackupEligible: boolean | null` on `AnonAuthState`; populated from `register()` and `login()` response.
...
- [x] **ANALYTICS-06**: Failure events (`*.finish.failure`) are emitted by default. README documents that consumers consuming analytics in privacy-restricted environments may filter by `event.type` if they want success-only.
