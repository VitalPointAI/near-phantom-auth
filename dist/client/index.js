...
function AnonAuthProvider({ apiUrl, passkey, children }) {
  ...
  const checkUsername = useCallback(async (username) => {
    try {
      ...
      return { available: false, suggestion: void 0 };
    }
  }, [api]);
  ...
  return /* @__PURE__ */ jsx(AnonAuthContext.Provider, { value, children });
}
...
function OAuthProvider({
  ...
  const handleCallback = useCallback(async (provider, code, state2) => {
    setState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
...
export { AnonAuthProvider, OAuthProvider, authenticateWithPasskey, createApiClient, createPasskey, isLikelyCloudSynced, isPlatformAuthenticatorAvailable, isWebAuthnSupported, useAnonAuth, useOAuth, useOAuthCallback };
