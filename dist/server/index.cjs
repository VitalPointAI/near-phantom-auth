...
var signers = require('@near-js/signers');
var crypto$2 = require('@near-js/crypto');
var utils = require('@near-js/utils');
...
var pino3__default = /*#__PURE__*/_interopDefault(pino3);
var bs582__default = /*#__PURE__*/_interopDefault(bs582);
...
function verifySessionId(signedValue, secret) {
  ...
  if (!crypto$1.timingSafeEqual(sigBuffer, expectedBuffer)) return null;
  return sessionId;
}
...
function createPasskeyManager(db, config) {
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "passkey" });
...
var _log = pino3__default.default({ level: "silent" }).child({ module: "wallet-recovery" });
...
function verifyWalletSignature(signature, expectedMessage) {
  try {
    ...
    const publicKeyBytes = bs582__default.default.decode(pubKeyStr);
...
function createWalletRecoveryManager(config) {
  (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "wallet-recovery" });
...
async function fundAccountFromTreasury(accountId, treasuryAccount, keyPair, amountNear, networkId, log2) {
  ...
  try {
    ...
    const yoctoStr = utils.parseNearAmount(amountNear);
    ...
    const transaction = buildTransferTransaction(
      ...
      blockHash,
      amountYocto,
      bs582__default.default
    );
    const txHash = crypto$1.createHash("sha256").update(transaction).digest();
...
var MPCAccountManager = class {
  ...
  constructor(config) {
    ...
    if (config.treasuryPrivateKey) {
      this.keyPair = crypto$2.KeyPair.fromString(config.treasuryPrivateKey);
    }
    this.fundingAmount = config.fundingAmount || "0.01";
    this.derivationSalt = config.derivationSalt;
    this.log = (config.logger ?? pino3__default.default({
      level: "silent",
      redact: {
        paths: [
  ...
  async createAccount(userId) {
    ...
    const seed = crypto$1.createHash("sha256").update(seedInput).digest();
  ...
  async addRecoveryWallet(nearAccountId, recoveryWalletPublicKey) {
    ...
    try {
      ...
      const blockHashBytes = bs582__default.default.decode(accessKeyResult.result.block_hash);
      const { addKey, fullAccessKey } = transactions.actionCreators;
      const recoveryPublicKey = crypto$2.PublicKey.fromString(recoveryWalletPublicKey);
...
function createIPFSRecoveryManager(config) {
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "ipfs-recovery" });
...
function createEmailService(config, log2) {
  const client = new clientSes.SESClient({
...
function wrapAnalytics(hook, opts = {}) {
  const log2 = (opts.logger ?? pino3__default.default({ level: "silent" })).child({ module: "analytics" });
...
var registerStartBodySchema = zod.z.object({});
var registerFinishBodySchema = zod.z.object({
  challengeId: zod.z.string().min(1),
  tempUserId: zod.z.string().min(1),
  codename: zod.z.string().min(1),
  ...
  sealingKeyHex: zod.z.string().regex(/^[0-9a-f]{64}$/).optional(),
  response: zod.z.object({
    id: zod.z.string().min(1),
    rawId: zod.z.string().min(1),
    type: zod.z.literal("public-key"),
    response: zod.z.object({
      clientDataJSON: zod.z.string().min(1),
      attestationObject: zod.z.string().min(1)
    }).passthrough(),
    ...
    clientExtensionResults: zod.z.object({}).catchall(zod.z.unknown())
  }).passthrough()
  // allow authenticatorAttachment and other vendor keys
});
var loginStartBodySchema = zod.z.object({
  codename: zod.z.string().min(1).optional()
});
var loginFinishBodySchema = zod.z.object({
  challengeId: zod.z.string().min(1),
  // WR-02: sealingKeyHex is validated here (64-char lowercase hex, 32-byte PRF
  // output) but NOT consumed by any route handler in this library. It is
  // accepted so downstream consumers (e.g., a DEK auth-service) can forward
  ...
  response: zod.z.object({
    ...
    response: zod.z.object({
      clientDataJSON: zod.z.string().min(1),
      authenticatorData: zod.z.string().min(1),
      signature: zod.z.string().min(1),
      userHandle: zod.z.string().optional()
    }).passthrough(),
...
var logoutBodySchema = zod.z.object({});
var walletLinkBodySchema = zod.z.object({});
var walletVerifyBodySchema = zod.z.object({
  ...
  walletAccountId: zod.z.string().min(1)
});
var walletStartBodySchema = zod.z.object({});
var walletFinishBodySchema = zod.z.object({
  signature: zod.z.object({
    signature: zod.z.string().min(1),
    publicKey: zod.z.string().min(1),
    message: zod.z.string().min(1)
  ...
  challenge: zod.z.string().min(1),
  nearAccountId: zod.z.string().min(1)
});
var ipfsSetupBodySchema = zod.z.object({
  password: zod.z.string().min(1)
});
var ipfsRecoverBodySchema = zod.z.object({
  cid: zod.z.string().min(1),
  password: zod.z.string().min(1)
});
var oauthCallbackBodySchema = zod.z.object({
  code: zod.z.string().min(1),
  state: zod.z.string().min(1)
});
var oauthLinkBodySchema = zod.z.object({
  code: zod.z.string().min(1),
  state: zod.z.string().min(1).optional(),
  codeVerifier: zod.z.string().min(1).optional()
});

// src/server/oauth/router.ts
function createOAuthRouter(config) {
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "oauth" });
  ...
  const authLimiter = expressRateLimit.rateLimit({
...
function createAuthMiddleware(sessionManager, db, logger) {
  const log2 = (logger ?? pino3__default.default({ level: "silent" })).child({ module: "middleware" });
...
function createRouter(config) {
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "router" });
  const router = express.Router();
  const {
    db,
    sessionManager,
  ...
  const recoveryLimiter = expressRateLimit.rateLimit({
  ...
  if (config.csrf) {
    const { generateCsrfToken, doubleCsrfProtection } = csrfCsrf.doubleCsrf({
      getSecret: () => config.csrf.secret,
      getSessionIdentifier: (req) => req.ip ?? "",
      cookieName: "__Host-csrf",
      cookieOptions: {
...
function createEnterpriseBinding(config) {
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "enterprise" });
  ...
  const api = {
    async linkIdentity(input) {
      ...
      if (!nearAccountId) {
        ...
        const mpcAccount = await mpcManager.createAccount(crypto$1.randomUUID());
        nearAccountId = mpcAccount.nearAccountId;
      }
...
var scimUserSchema = zod.z.object({
  userName: zod.z.string().min(1),
  externalId: zod.z.string().min(1).optional(),
  active: zod.z.boolean().optional(),
  name: zod.z.record(zod.z.string(), zod.z.unknown()).optional(),
  emails: zod.z.array(zod.z.record(zod.z.string(), zod.z.unknown())).optional(),
  groups: zod.z.array(zod.z.union([
    zod.z.string(),
    zod.z.object({ value: zod.z.string().optional(), display: zod.z.string().optional() }).passthrough()
  ])).optional()
}).passthrough();
var scimPatchSchema = zod.z.object({
  Operations: zod.z.array(zod.z.object({
    op: zod.z.string().min(1),
    path: zod.z.string().optional(),
    value: zod.z.unknown().optional()
  }).passthrough())
}).passthrough();
var scimGroupSchema = zod.z.object({
  displayName: zod.z.string().min(1),
  externalId: zod.z.string().optional(),
  members: zod.z.array(zod.z.object({
...
function tokenMatches(actual, expected) {
  ...
  return crypto$1.timingSafeEqual(actualBuffer, expectedBuffer);
}
...
function createScimRouter(config) {
  const router = express.Router();
  const log2 = (config.logger ?? pino3__default.default({ level: "silent" })).child({ module: "scim" });
  ...
  const authRateConfig = config.rateLimiting?.auth ?? {};
  router.use(expressRateLimit.rateLimit({
    windowMs: authRateConfig.windowMs ?? 15 * 60 * 1e3,
    limit: authRateConfig.limit ?? 20,
    standardHeaders: "draft-8",
    legacyHeaders: false,
  ...
  router.post("/Groups", (req, res) => {
    ...
    const id = parsed.data.externalId || crypto$1.randomUUID();
    const group = {
      id,
...
var log = pino3__default.default({ level: "silent" }).child({ module: "webauthn" });
...
function createAnonAuth(config) {
  const logger = config.logger ?? pino3__default.default({ level: "silent" });
  let db;
...
exports.POSTGRES_ENTERPRISE_SCHEMA = POSTGRES_ENTERPRISE_SCHEMA;
...
exports.createEnterpriseBinding = createEnterpriseBinding;
...
exports.createScimRouter = createScimRouter;
