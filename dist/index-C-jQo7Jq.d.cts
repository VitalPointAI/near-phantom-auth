...
type RegisterFailureReason = 'invalid-codename' | 'passkey-verification-failed' | 'internal-error';
/** Failure reasons for `login.finish.failure`. Static enum — NEVER `Error.message`. */
type LoginFailureReason = 'auth-failed' | 'user-not-found' | 'internal-error';
 ...
 */
type AfterAuthSuccessProvider = 'google' | 'github' | 'twitter';
/**
 ...
 */
type BackfillReason = 'already-current' | 'no-legacy-data' | 'completed' | 'skipped';
/**
 ...
 */
interface BackfillKeyBundleCtx {
    userId: string;
    codename: string;
    nearAccountId: string;
 ...
 */
interface BackfillKeyBundleResult {
    backfilled: boolean;
    reason?: BackfillReason;
}
interface AnonAuthHooks {
    /**
